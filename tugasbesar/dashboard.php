...
if(isset($_POST['bsimpan']))
{
	if($_GET['hal'] == "edit") 
	{
		...
		{
			$simpan = mysqli_query($conn, "INSERT INTO mhs (nim, nama, prodi) VALUES ('$_POST[tnim]',
				'$_POST[tnama]',
				'$_POST[tprodi]')
				");
			if ($simpan) {
...
<body>
	<nav class="navbar navbar-expand-1g navbar-light bg-warning fixed-top">
		<div class="container-fluid">
			<a class="navbar-brand" href="#"><b>WEB KAMPUS UNIVERSITAS IBRAHIMY</b>
			</a>
			<div class="collapse navbar-collapse" id="navbarSupportedContent">
				<ul class="navbar-nav me-auto mb-2 mb-lg-0">
				<li class="nav-item">
			<a class="nav-link active" aria-current="page" href="index.php" onclick="
			return confirm('yakin ingin keluar')">Logout</a>
			</li>
			</ul>
			<form class="d-flex">
...
<div class="container">
	...
	<br/>
		...
		<div class="card mt-3">
			...
			MASUKKAN DATA MAHASISWA DIBAWAH
			</div>
		<div class="card-body">
			<form method="post" action="">
			<div class="form-group">
			<label>Nim</label>
			<input type="text" name="tnim" value="<?=@$nim?>" class="form-control"
		...
		<div class="form-group">
			...
			<input type="text" name="tprodi" value="<?=@$prodi?>" class="form-control"
			placeholder="Input Prodi anda disini!" required>
			</div>
...
<div class="card-body">I<table class="table table-bordered table-striped">
		<tr>
			<th>No.</th>
			<th>Nim</th>
			<th>Nama</th>
...
?>
		<tr>
			...
			<td><?=$data['nim'] ?></td>
			<td><?=$data['nama'] ?></td>
			<td><?=$data['prodi'] ?></td>I<td>
		<a href="dashboard.php?hal=edit&id=<?=$data['no'] ?>" class="btn btn-warning"> Edit </a>
		<a href="dashboard.php?hal=hapus&id=<?=$data['no'] ?>"onclick="return confirm ('Apakah yakin ingin menghapus data ini?')" class="btn btn-danger"> Hapus </a>
			</td>
		</tr>
		<?php endwhile; ?>
		</table>
